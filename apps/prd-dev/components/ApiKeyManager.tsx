...
export default function ApiKeyManager({ apiKeys, onUpdateKeys }: ApiKeyManagerProps) {
  ...
  const validateKey = (key: string, provider: 'openai' | 'anthropic' | 'gemini') => {
    if (!key) return null
    if (provider === 'openai') {
    ...
    if (provider === 'gemini') {
      return key.length > 20 ? 'valid' : 'invalid' // Gemini keys are typically longer
    }
    return null
  }
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700 mt-1">
                You're currently in demo mode with simulated AI responses. Add an API key below to unlock full AI capabilities with OpenAI, Claude, or Gemini.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            ...
            />
            <div className="absolute inset-y-0 right-0 pr-3 flex items-center gap-2">
              {validateKey(tempKeys.gemini || '', 'gemini') === 'valid' && (
                <CheckCircle className="text-green-500" size={18} />
              )}
              {validateKey(tempKeys.gemini || '', 'gemini') === 'invalid' && (
                <AlertCircle className="text-red-500" size={18} />
              )}
              <button
                type="button"
